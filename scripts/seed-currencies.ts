...
async function seed() {
  ...
  for (let i = 0; i < currencies.length; i += batchSize) {
    ...
    for (const c of batch) {
      await sql`INSERT INTO currencies (code, name, symbol) VALUES (${c.code}, ${c.name}, ${c.symbol}) ON CONFLICT (code) DO NOTHING`;
    }

    console.log(`Inserted batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(currencies.length / batchSize)}`);
  }

  const count = await sql`SELECT COUNT(*) as count FROM currencies`;
  console.log(`Successfully seeded ${count[0].count} currencies`);
  await sql.end();
}
