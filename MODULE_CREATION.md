...
- **Document print layouts**: e.g. receivings PDF — code in `handlePrint` inside the detail client (`components/receivings/receiving-detail-client.tsx`), not a separate HTML print page. Header branding from `getAppSettings()` (app name, logo, address, phone). Edit print columns in that `autoTable` config.
- **Document workflows**: when status is not Active/Deleted (e.g. receivings `Draft`/`Completed`/`Cancelled`), use a **new** `pgEnum` for that document only; audit columns stay **`deleted_at`/`deleted_by`/`deleted_reason`**. List filters must match the document enum values. Receivings list UI **defaults to Draft**.
