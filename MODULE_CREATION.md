...
**Reference modules**: `departments` (simple), `roles` (has clone modal + detail page with a child table + **two-column detail layout**), `products` (wide table, required FKs `uomId`/`gstTypeId`, editable unique code + next-code prefill, Created At omitted), `payment_methods` (simple text module with **dual unique fields** `name` + `description`, audit fields, Created At shown), `payment_terms` (unique `name` + integer `termDays` default 0 via `z.coerce.number()`, audit fields), `trans_types` (simplest lookup: unique `name` + status + audit fields), `stock_levels` (**read-only** join view: unique product+location qty, GET APIs only; **Updated By** via `formatUserDisplay`), `stock_movements` (**read-only** signed-qty trail; **Created By** via `formatUserDisplay`; DatePicker filters), `receivings` (**master-detail** workflow via **`inventoryStatusEnum`**; stock posting `db.transaction`; list Draft default; print via **`printDocumentPdf`**), `releasings` (**stock-out** twin: RLS/RI numbers, scan **auto-add** items, stock check at from-location, same **`printDocumentPdf`** layout), `transfers` (**location-to-location**: both locations required; complete ±qty on two stock_levels legs; no products.stock update; TRAN/TI numbers; scan auto-add), `adjustments` (**single-line stock adj**: no Draft; **uomId** snapshot from product; qty_old from stock_levels; qty_new = old+adj; preview API for form UOM/qtyOld; save posts Completed; cancel reverses sign; View modal **2-col** (Product/Remarks full width) + Print), `conversions` (**product→product** at one location: CNV-#####; auto UOMs; dual stock_levels/products legs on save/cancel; from ≠ to).
...
`home`, `settings`, `users`, `shield`, `file-text`, `bar-chart-3`, `calendar`, `mail`, `bell`, `search`, `layout-dashboard`, `lock`, `globe`, `heart`, `star`, `bookmark`, `tag`, `folder`, `image`, `video`, `music`, `phone`, `map-pin`, `shopping-cart`, `credit-card`, `truck`.
...
- [ ] `components/<plural>/<plural>-columns.tsx` — apply Created At rule: if data columns (excluding `#` and Actions) > 5, omit `createdAt` column; join/display FK `*Name` columns as needed; date cells use **`formatDateOnly`** (`YYYY-MM-DD`); Created/Updated/Deleted By use **`formatUserDisplay`** (`[lastname], [F].`) when APIs provide `*Display` fields
- [ ] `components/<plural>/<plural>-table.tsx` (accepts `timezone` prop if showing dates; PDF/Excel/mobile use `formatDateOnly`; user columns use display name not email)
...
- **User display**: Created/Updated/Deleted By use **`formatUserDisplay`** from **`lib/format-user.ts`** → **`[lastname], [F].`**. Join/select `users.firstname` + `lastname`; APIs expose `createdByDisplay` / `updatedByDisplay` / `deletedByDisplay`. Do not show email on inventory/transaction UIs.
- **Inventory decimals**: qty / unit cost / total use schema **`decimal(10, scale: 4)`** and UI **`0.0000`**; right-align those columns. Receivings items + print follow this.
- **View modals**: prefer **2-column** field grid (`grid-cols-1 sm:grid-cols-2`); long fields (product name, remarks) use `sm:col-span-2`. Pattern: `components/adjustments/adjustment-view-cancel-modals.tsx`.
- **Modal scroll shell (standard for all modals — applied app-wide)**: header and footer stay pinned when body content overflows the viewport. `DialogContent`: `showCloseButton={false}` + `flex max-h-[90vh] flex-col` + existing size/border/`p-0`. Body only: `min-h-0 flex-1 overflow-y-auto` + existing layout classes. Form modals: form `flex min-h-0 flex-1 flex-col`, fields in scroll wrapper, footer pinned as last child. Do **not** scroll the whole `DialogContent`. Reference: `components/conversions/` and any module modal.
- **Document print layouts**: use shared **`printDocumentPdf`** from **`lib/print/document-print.ts`** for all document PDFs (receivings, releasings, future). Pass documentNo, title, fields, table columns/rows, signatures, `appSettings`, timezone, fileName. Do **not** duplicate jsPDF chrome per module.
- **Document workflows**: when status is not Active/Deleted (e.g. receivings/releasings `Draft`/`Completed`/`Cancelled`), use shared **`inventoryStatusEnum`** / PG type **`inventory_status`** on inventory document tables; audit columns stay **`deleted_at`/`deleted_by`/`deleted_reason`**. List filters must match the document enum values. Receivings/releasings list UI **defaults to Draft**.
- **Master-detail documents**: follow **receivings** — master form first; child items on `/[id]` (roles two-column layout); status-gated actions; stock side effects in **`db.transaction`** (`lib/receiving-stock.ts` pattern); seed any trans types needed for stock_movements.
- **FEFO / lots (required for stock posting)**: use **`lib/fefo.ts`** — `upsertInventoryBatch`, `consumeFefo` (earliest `dateExpiry` first, **null last**), `previewFefo`. Receiving **complete** creates `inventory_batches` from item `batchNo` + `dateExpiry`. Outbound completes (releasing/transfer/adjust−/conversion) **consume FEFO** and write allocation tables / movement `batchId`/`batchNo`. Invariant: `SUM(inventory_batches.qty) = stock_levels.qty` per product+location. Never reduce only `stock_levels` without lots. Trans types: `Receiving`, `Receiving Cancel`, `Releasing`, `Releasing Cancel`, `Transfer`, `Transfer Cancel`, `Adjustment`, **`Adjustment Cancel`**, `Conversion`, `Conversion Cancel`. Reset stock history: `npm run db:truncate-inventory`. Design: `FEFO-STUDY.md`.
- **List PDF company header**: report/list exports that need branding use **`drawCompanyHeader`** from `lib/print/document-print.ts` (logo, appName, address, `Tel:`) + `appSettings` from `getAppSettings()`. Document forms still use **`printDocumentPdf`**.
