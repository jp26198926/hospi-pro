...
- **Document workflows**: when status is not Active/Deleted (e.g. receivings `Draft`/`Completed`/`Cancelled`), use a **new** `pgEnum` for that document only; audit columns stay **`deleted_at`/`deleted_by`/`deleted_reason`**. List filters must match the document enum values.
