...
export async function GET(request: NextRequest) {
  try {
    const auth = await requirePermission(request, "/phar-stock-movements", "Read");
    ...
    const conditions = [eq(stockMovements.locationId, phar.id)];
    ...
    console.error("GET /api/phar-stock-movements error:", error);
    return Response.json(
      { error: "Failed to fetch pharmacy stock movements" },
      { status: 500 }
    );
  }
}
