...
export async function GET(request: NextRequest) {
  try {
    ...
    const conditions = [eq(adjustments.locationId, phar.id)];
    ...
    console.error("GET /api/phar-adjustments error:", error);
    return Response.json({ error: "Failed to fetch pharmacy adjustments" }, { status: 500 });
  }
}

export async function POST(request: NextRequest) {
  try {
    const auth = await requirePermission(request, "/phar-adjustments", "Add");
    if (auth instanceof Response) return auth;
    ...
    console.error("POST /api/phar-adjustments error:", error);
    return Response.json({ error: "Failed to create pharmacy adjustment" }, { status: 500 });
  }
}
