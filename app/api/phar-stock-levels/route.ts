...
import { eq, desc, asc, ilike, and, or, gte, lte, sql, count as drizzleCount } from "drizzle-orm";
...
export async function GET(request: NextRequest) {
  try {
    const auth = await requirePermission(request, "/phar-stock-levels", "Read");
    ...
    const conditions = [eq(stockLevels.locationId, phar.id)];
    ...
    console.error("GET /api/phar-stock-levels error:", error);
    return Response.json({ error: "Failed to fetch pharmacy stock levels" }, { status: 500 });
  }
}
