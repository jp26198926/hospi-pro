...
interface GstTypeSearchModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSearch: (term: string, status: string) => void;
}

export function GstTypeSearchModal({
  open,
  onOpenChange,
  onSearch,
}: GstTypeSearchModalProps) {
      ...
      <DialogContent showCloseButton={false} className="flex max-h-[90vh] flex-col rounded-sm border-[#ddd] p-0 sm:max-w-md">
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4">
          <div className="space-y-2">
            <Label htmlFor="search-gst-type" className="text-sm font-medium text-[#333]">
              Code or Name
            </Label>
            <Input
              id="search-gst-type"
