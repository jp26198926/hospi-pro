...
interface GstTypeDeleteModalProps {
  open: boolean;
...
export function GstTypeDeleteModal({
  open,
  onOpenChange,
  gstType,
  onSuccess,
}: GstTypeDeleteModalProps) {
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/gst-types/${gstType.id}`, {
        method: "DELETE",
      ...
      if (!res.ok) {
        toast.error(json.error || "Failed to delete GST type");
        return;
      }

      toast.success("GST type deleted successfully");
      setReason("");
      onOpenChange(false);
      onSuccess();
    } catch {
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm text-[#333]">
                Are you sure you want to delete <strong className="text-[#337ab7]">{gstType?.code} — {gstType?.name}</strong>?
              </p>
