...
interface ColumnActions {
  onView: (gstType: GstType) => void;
  onEdit: (gstType: GstType) => void;
  onDelete: (gstType: GstType) => void;
  onRestore: (gstType: GstType) => void;
  timezone: string;
}

export function getColumns({ onView, onEdit, onDelete, onRestore, timezone }: ColumnActions): ColumnDef<GstType>[] {
  return [
    {
      accessorKey: "no",
      header: "#",
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="font-medium text-[#337ab7]">{row.original.code}</span>
      ),
    },
    {
      accessorKey: "name",
    ...
    },
    {
      ...
      size: 120,
      cell: ({ row }) => {
        const gstType = row.original;
        return (
          <div className="flex items-center gap-1">
            <button
              onClick={() => onView(gstType)}
              title="View"
