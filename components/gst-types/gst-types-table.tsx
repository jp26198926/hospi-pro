...
import { GstTypeSearchModal } from "./gst-type-search-modal";

export function GstTypesTable({ timezone }: { timezone: string }) {
  const router = useRouter();
  const [data, setData] = useState<GstType[]>([]);
  ...
  const [editGstType, setEditGstType] = useState<GstType | null>(null);
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [deleteGstType, setDeleteGstType] = useState<GstType | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/gst-types?${params}`);
      ...
      console.error("Failed to fetch GST types:", error);
    } finally {
      setLoading(false);
    }
  ...
  const handleView = (gstType: GstType) => {
    router.push(`/gst-types/${gstType.id}`);
  };

  const handleEdit = (gstType: GstType) => {
    setEditGstType(gstType);
    setFormModalMode("edit");
    setFormModalOpen(true);
  };

  const handleDelete = (gstType: GstType) => {
    setDeleteGstType(gstType);
    setDeleteModalOpen(true);
  };

  const handleRestore = async (gstType: GstType) => {
    try {
      const res = await fetch(`/api/gst-types/${gstType.id}`, { method: "PATCH" });
      const json = await res.json();

      if (!res.ok) {
        toast.error(json.error || "Failed to restore GST type");
        return;
      }

      toast.success("GST type restored successfully");
      fetchData();
    } catch {
  ...
  const exportPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(16);
    doc.text("GST Types Report", 14, 20);

    autoTable(doc, {
      startY: 30,
      head: [["#", "Code", "Name", "Status", "Created At", "Updated At"]],
      body: data.map((gt, idx) => [
        idx + 1,
        gt.code,
        gt.name,
        gt.status,
    ...
    doc.save("gst-types.pdf");
  };

  const exportExcel = () => {
    const worksheetData = data.map((gt, idx) => ({
      "#": idx + 1,
      Code: gt.code,
      Name: gt.name,
      Status: gt.status,
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "GST Types");
    XLSX.writeFile(workbook, "gst-types.xlsx");
  };
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2">
          <Button
            onClick={() => { setFormModalMode("add"); setEditGstType(null); setFormModalOpen(true); }}
      ...
      <div className="space-y-3 md:hidden">
        {loading ? (
          ...
          data.map((gstType, index) => (
            <div key={gstType.id} className="border border-[#ddd] bg-white">
