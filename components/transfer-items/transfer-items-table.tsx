...
import {
  getTransferItemColumns,
  TransferItem,
} from "./transfer-items-columns";
import { TransferItemFormModal } from "./transfer-item-form-modal";
import { TransferItemActionModal } from "./transfer-item-action-modal";
import type { TransferStatus } from "@/components/transfers/transfers-columns";

interface TransferItemsTableProps {
  transferId: number;
  masterStatus: TransferStatus;
  timezone: string;
  onMutated?: () => void;
  reloadKey?: number;
}
...
export function TransferItemsTable({
  ...
  const [actionItem, setActionItem] = useState<TransferItem | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/transfer-items?${params}`);
  ...
  }, [transferId, showCancelled]);

  useEffect(() => {
    fetchData();
  }, [fetchData, reloadKey]);

  const columns = getTransferItemColumns({
    masterStatus,
    timezone,
    onEdit: (item) => {
      setEditItem(item);
