...
export function getTransferItemColumns({
  masterStatus,
  timezone,
  onEdit,
  onCancel,
  ...
  onRestore: (item: TransferItem) => void;
}): ColumnDef<TransferItem>[] {
  return [
    {
      ...
      cell: ({ row }) => (
        <span className="font-medium text-[#337ab7]">
          {row.original.seriesNo || `TI-${String(row.original.id).padStart(6, "0")}`}
        </span>
      ),
    },
    {
