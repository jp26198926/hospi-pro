...
export function PermissionsTable() {
  ...
  const exportPDF = () => {
    ...
    autoTable(doc, {
      ...
      body: data.map((perm, idx) => [
        ...
        perm.permission,
        perm.status,
        format(new Date(perm.createdAt), "MMM dd, yyyy HH:mm"),
        perm.updatedAt ? format(new Date(perm.updatedAt), "MMM dd, yyyy HH:mm") : "-",
      ]),
    });
  ...
  const exportExcel = () => {
    const worksheetData = data.map((perm, idx) => ({
      ...
      Status: perm.status,
      "Created At": format(new Date(perm.createdAt), "MMM dd, yyyy HH:mm"),
      "Updated At": perm.updatedAt ? format(new Date(perm.updatedAt), "MMM dd, yyyy HH:mm") : "-",
    }));
            ...
            <div key={permission.id} className="border border-[#ddd] bg-white">
              ...
              <div className="px-4 py-3">
                ...
                <div className="mt-2 space-y-1 text-xs text-muted-foreground">
                  <p><span className="font-medium text-[#666]">Created:</span> {format(new Date(permission.createdAt), "MMM dd, yyyy HH:mm")}</p>
                  <p><span className="font-medium text-[#666]">Updated:</span> {permission.updatedAt ? format(new Date(permission.updatedAt), "MMM dd, yyyy HH:mm") : "-"}</p>
                </div>
              </div>
