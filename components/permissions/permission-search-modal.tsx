...
interface PermissionSearchModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSearch: (term: string, status: string) => void;
}

export function PermissionSearchModal({
  open,
  onOpenChange,
  onSearch,
}: PermissionSearchModalProps) {
      ...
      <DialogContent className="sm:max-w-md rounded-sm border-[#ddd] p-0">
        ...
        <div className="space-y-4 p-4">
          <div className="space-y-2">
            <Label htmlFor="search-permission" className="text-sm font-medium text-[#333]">
              Permission Name
            </Label>
            <Input
