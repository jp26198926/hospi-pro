...
import {
  ...
  Conversion,
  fmtQty,
} from "@/components/conversions/conversions-columns";
import { PharConversionFormModal } from "./phar-conversion-form-modal";
import { PharConversionSearchModal } from "./phar-conversion-search-modal";
import {
  PharConversionViewModal,
  PharConversionCancelModal,
} from "./phar-conversion-view-cancel-modals";

interface PharConversionsTableProps {
  timezone: string;
  appSettings?: {
    appName: string;
    appLogo: string | null;
...
export function PharConversionsTable({ timezone, appSettings }: PharConversionsTableProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/phar-conversions?${params}`);
      ...
      console.error("Failed to fetch pharmacy conversions:", error);
    } finally {
      setLoading(false);
    }
  }, [page, limit, sorting, search, statusFilter, dateFrom, dateTo]);
  ...
  const exportPDF = () => {
    ...
    doc.setFontSize(16);
    doc.text("Phar Conversions Report", 14, 20);
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Phar Conversions");
    XLSX.writeFile(workbook, "phar-conversions.xlsx");
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <PharConversionFormModal
        open={formOpen}
        onOpenChange={setFormOpen}
        onSuccess={fetchData}
      />
      <PharConversionSearchModal
        open={searchOpen}
