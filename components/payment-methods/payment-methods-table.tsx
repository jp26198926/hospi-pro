...
import { getColumns, PaymentMethod } from "./payment-methods-columns";
...
import { PaymentMethodSearchModal } from "./payment-method-search-modal";

export function PaymentMethodsTable({ timezone }: { timezone: string }) {
  ...
  const handleRestore = async (paymentMethod: PaymentMethod) => {
    try {
      ...
      if (!res.ok) {
        toast.error(json.error || "Failed to restore payment method");
        return;
      }
  ...
  const exportPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(16);
    doc.text("Payment Methods Report", 14, 20);

    autoTable(doc, {
      startY: 30,
      head: [["#", "Name", "Description", "Status", "Created At", "Updated At"]],
  ...
  const exportExcel = () => {
    ...
    XLSX.writeFile(workbook, "payment-methods.xlsx");
  };
