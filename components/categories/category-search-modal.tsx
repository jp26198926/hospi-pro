...
interface CategorySearchModalProps {
  ...
  onSearch: (term: string, type: string, status: string) => void;
}
...
export function CategorySearchModal({
  ...
  const handleSearch = () => {
    onSearch(searchTerm, typeFilter, statusFilter);
    onOpenChange(false);
  };
      ...
      <DialogContent className="sm:max-w-md rounded-sm border-[#ddd] p-0">
        ...
        <div className="space-y-4 p-4">
          <div className="space-y-2">
            <Label htmlFor="search-name" className="text-sm font-medium text-[#333]">
              Name
            </Label>
            <Input
              id="search-name"
