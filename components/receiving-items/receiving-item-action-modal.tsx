...
interface ReceivingItemActionModalProps {
  open: boolean;
...
export function ReceivingItemActionModal({
  open,
  onOpenChange,
  action,
  item,
  onSuccess,
}: ReceivingItemActionModalProps) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/receiving-items/${item.id}`, {
        method,
        headers:
      ...
      toast.success(
        action === "cancel" ? "Item cancelled" : "Item restored"
      );
      setReason("");
      onOpenChange(false);
      onSuccess();
