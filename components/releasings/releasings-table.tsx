...
import { getColumns, Releasing } from "./releasings-columns";
import { ReleasingFormModal } from "./releasing-form-modal";
import { ReleasingSearchModal } from "./releasing-search-modal";

export function ReleasingsTable({ timezone }: { timezone: string }) {
  const router = useRouter();
  const [data, setData] = useState<Releasing[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      console.error("Failed to fetch releasings:", error);
    } finally {
      setLoading(false);
    }
  ...
  const columns = getColumns({
    onView: (row) => router.push(`/releasings/${row.id}`),
    timezone,
  });
  ...
  const exportPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(16);
    doc.text("Releasings Report", 14, 20);
    ...
    autoTable(doc, {
      ...
      body: data.map((row, idx) => [
        idx + 1,
        row.transNo || `RLS-${String(row.id).padStart(5, "0")}`,
  ...
  const exportExcel = () => {
    const worksheetData = data.map((row, idx) => ({
      "#": idx + 1,
      "Trans #": row.transNo || `RLS-${String(row.id).padStart(5, "0")}`,
            ...
            <div key={item.id} className="border border-[#ddd] bg-white">
              <div className="flex items-center justify-between border-b border-[#eee] bg-[#f8f8f8] px-4 py-2">
                <span className="text-xs font-medium text-[#337ab7]">
                  {item.transNo ||
                    `RLS-${String(item.id).padStart(5, "0")}`}
                </span>
                <span
              ...
              <div className="px-4 py-3">
                <p className="text-sm font-semibold">{item.receiverName}</p>
              ...
              <div className="flex border-t border-[#eee]">
                <button
                  onClick={() => router.push(`/releasings/${item.id}`)}
