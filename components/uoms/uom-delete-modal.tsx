...
export function UomDeleteModal({
  open,
  onOpenChange,
  uom,
  onSuccess,
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(json.error || "Failed to delete UOM");
        return;
      }
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm text-[#333]">
                Are you sure you want to delete <strong className="text-[#337ab7]">{uom?.code} — {uom?.name}</strong>?
              </p>
