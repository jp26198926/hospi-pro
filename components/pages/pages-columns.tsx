...
interface ColumnActions {
  onView: (page: PageRecord) => void;
  onEdit: (page: PageRecord) => void;
  onDelete: (page: PageRecord) => void;
  onRestore: (page: PageRecord) => void;
}

export function getColumns({ onView, onEdit, onDelete, onRestore }: ColumnActions): ColumnDef<PageRecord>[] {
  return [
    {
      accessorKey: "no",
      header: "#",
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="font-medium text-[#337ab7]">{row.original.page}</span>
      ),
    },
    {
      accessorKey: "path",
    ...
    },
    {
      ...
      header: "Parent",
      cell: ({ row }) => (
        <span className="text-sm text-muted-foreground">{row.original.parentName || "-"}</span>
      ),
    },
    {
      accessorKey: "status",
