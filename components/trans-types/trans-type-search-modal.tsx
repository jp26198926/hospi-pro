...
interface TransTypeSearchModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSearch: (term: string, status: string) => void;
}

export function TransTypeSearchModal({
  open,
  onOpenChange,
  onSearch,
}: TransTypeSearchModalProps) {
      ...
      <DialogContent className="sm:max-w-md rounded-sm border-[#ddd] p-0">
        ...
        <div className="space-y-4 p-4">
          <div className="space-y-2">
            <Label htmlFor="search-trans-type" className="text-sm font-medium text-[#333]">
              Name
            </Label>
            <Input
