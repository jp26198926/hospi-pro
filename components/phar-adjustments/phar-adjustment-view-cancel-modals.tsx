...
interface PharAdjustmentViewModalProps {
  open: boolean;
...
interface PharAdjustmentCancelModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  adjustment: Adjustment | null;
  onSuccess: () => void;
...
export function PharAdjustmentViewModal({
  open,
  onOpenChange,
  adjustment,
  timezone,
  appSettings,
}: PharAdjustmentViewModalProps) {
  ...
  const handlePrint = async () => {
    ...
    await printDocumentPdf({
      ...
      fieldsRight: [
        {
          label: "Status",
          badge: adjustment.status === "Cancelled" ? "Cancelled" : "Completed",
        },
      ],
...
export function PharAdjustmentCancelModal({
  open,
  onOpenChange,
  adjustment,
  onSuccess,
}: PharAdjustmentCancelModalProps) {
  ...
  const handleConfirm = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/phar-adjustments/${adjustment.id}`, {
        method: "DELETE",
