...
interface PharAdjustmentFormModalProps {
  open: boolean;
...
export function PharAdjustmentFormModal({
  open,
  onOpenChange,
  onSuccess,
  pharmacyLocation,
}: PharAdjustmentFormModalProps) {
  ...
  const locationId = pharmacyLocation?.id ?? null;
  const locationName = pharmacyLocation?.name || "Pharmacy";
  const qtyNew = qtyOld !== null ? qtyOld + (Number(qtyAdj) || 0) : null;
  ...
  useEffect(() => {
    ...
    async function preview() {
      try {
        const res = await fetch(`/api/phar-adjustments/preview?productId=${productId}`);
        const json = await res.json();
        if (!res.ok || cancelled) return;
  ...
  const onSubmit = async () => {
    if (!date || !locationId || !productId) {
      toast.error("Date, pharmacy location, and product are required");
      return;
    }
    ...
    setSaving(true);
    try {
      const res = await fetch("/api/phar-adjustments", {
        method: "POST",
      ...
      toast.success(
        `Phar adjustment ${json.data?.transNo || ""} saved — stock updated`
      );
      onOpenChange(false);
      onSuccess();
    } catch {
      ...
      >
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4">
          <div className="space-y-2">
            ...
            <DatePicker id="phar-adjustment-date" value={date} onValueChange={setDate} />
          </div>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <Input
                readOnly
                value={locationName}
                className="border-[#ccc] bg-[#f8f8f8]"
                title="Locked to pharmacy"
              />
              <p className="text-xs text-muted-foreground">Fixed to pharmacy</p>
            </div>
            <div className="space-y-2">
              <Label className="text-sm font-medium text-[#333]">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-4">
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-muted-foreground">+ or − (not zero)</p>
            </div>
            <div className="space-y-2">
