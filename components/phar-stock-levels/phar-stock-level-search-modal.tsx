...
interface PharStockLevelSearchModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSearch: (term: string, dateFrom: string, dateTo: string) => void;
}

export function PharStockLevelSearchModal({
  open,
  onOpenChange,
  onSearch,
}: PharStockLevelSearchModalProps) {
  const [searchTerm, setSearchTerm] = useState("");
  const [dateFrom, setDateFrom] = useState("");
  const [dateTo, setDateTo] = useState("");

  const handleSearch = () => {
    onSearch(searchTerm, dateFrom, dateTo);
    onOpenChange(false);
  };
      ...
      >
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4">
          <div className="space-y-2">
            <Label htmlFor="search-phar-stock" className="text-sm font-medium text-[#333]">
              Product Code or Name
            </Label>
            <Input
              id="search-phar-stock"
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="phar-date-from" className="text-sm font-medium text-[#333]">
                Date From
              </Label>
              <DatePicker
                id="phar-date-from"
            ...
            <div className="space-y-2">
              <Label htmlFor="phar-date-to" className="text-sm font-medium text-[#333]">
                Date To
              </Label>
              <DatePicker
                id="phar-date-to"
