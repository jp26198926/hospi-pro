...
import { getColumns, StockLevel, formatQty } from "@/components/stock-levels/stock-levels-columns";
import { PharStockLevelSearchModal } from "./phar-stock-level-search-modal";

export function PharStockLevelsTable({ timezone }: { timezone: string }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/phar-stock-levels?${params}`);
      ...
      console.error("Failed to fetch pharmacy stock levels:", error);
    } finally {
      setLoading(false);
    }
  }, [page, limit, sorting, search, dateFrom, dateTo]);
  ...
  const exportPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(16);
    doc.text("Phar Stock Level Report", 14, 20);
    autoTable(doc, {
      startY: 30,
      head: [
        ["#", "Product Code", "Product Name", "QTY", "Location", "Updated At", "Updated By"],
      ],
    ...
    doc.save("phar-stock-levels.pdf");
  };
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Phar Stock Levels");
    XLSX.writeFile(workbook, "phar-stock-levels.xlsx");
  };
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow>
                <TableCell colSpan={columns.length} className="h-24 text-center">
                  No pharmacy stock levels found.
                </TableCell>
              </TableRow>
            )}
          </TableBody>
          ...
          data.map((item, index) => (
            <div key={item.id} className="border border-[#ddd] bg-white">
              <div className="flex items-center justify-between border-b border-[#eee] bg-[#f8f8f8] px-4 py-2">
                ...
                <span className="bg-[#337ab7] px-2 py-0.5 text-xs font-medium text-white">
                  QTY {formatQty(item.qty)}
                </span>
              </div>
              <div className="px-4 py-3">
                <p className="text-base font-semibold text-[#337ab7]">{item.productCode}</p>
                <p className="text-sm text-[#333]">{item.productName}</p>
                <div className="mt-2 space-y-1 text-xs text-muted-foreground">
                  <p>Location: {item.locationName || "-"}</p>
                  <p>
                    Updated: {item.updatedAt ? formatDateOnly(item.updatedAt, timezone) : "-"}
                  </p>
                  <p>By: {item.updatedByDisplay || item.updatedByEmail || "-"}</p>
                </div>
              </div>
            </div>
          ))
