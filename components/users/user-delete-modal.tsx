    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent showCloseButton={false} className="flex max-h-[90vh] flex-col rounded-sm border-[#ddd] p-0 sm:max-w-md">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm text-[#333]">
                Are you sure you want to delete <strong className="text-[#337ab7]">{user?.email}</strong>?
              </p>
        ...
        <div className="flex justify-end gap-2 border-t border-[#eee] bg-[#f8f8f8] px-4 py-3">
          <Button variant="outline" onClick={() => onOpenChange(false)} disabled={loading} className="border-[#ccc] bg-white text-[#333] hover:bg-[#f5f5f5]">
            Cancel
          </Button>
          <Button onClick={handleDelete} disabled={loading} className="bg-[#d9534f] text-white hover:bg-[#c9302c]">
