...
export function DepartmentsTable() {
  ...
  const handleRestore = async (department: Department) => {
    try {
      const res = await fetch(`/api/departments/${department.id}`, { method: "PATCH" });
      const json = await res.json();

      if (!res.ok) {
        toast.error(json.error || "Failed to restore department");
        return;
      }
  ...
  const exportPDF = () => {
    ...
    autoTable(doc, {
      startY: 30,
      head: [["#", "Department", "Status", "Created At", "Updated At"]],
      body: data.map((dept, idx) => [
        idx + 1,
        dept.department,
        dept.status,
  ...
  const exportExcel = () => {
    const worksheetData = data.map((dept, idx) => ({
      "#": idx + 1,
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2">
          <Button
            onClick={() => { setFormModalMode("add"); setEditDepartment(null); setFormModalOpen(true); }}
      ...
      <div className="space-y-3 md:hidden">
        {loading ? (
          ...
          data.map((department, index) => (
            <div key={department.id} className="border border-[#ddd] bg-white">
