...
import { PaymentTermSearchModal } from "./payment-term-search-modal";

export function PaymentTermsTable({ timezone }: { timezone: string }) {
  ...
  const [editPaymentTerm, setEditPaymentTerm] = useState<PaymentTerm | null>(null);
  const [deleteModalOpen, setDeleteModalOpen] = useState(false);
  const [deletePaymentTerm, setDeletePaymentTerm] = useState<PaymentTerm | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/payment-terms?${params}`);
  ...
  const handleView = (paymentTerm: PaymentTerm) => {
    router.push(`/payment-terms/${paymentTerm.id}`);
  };

  const handleEdit = (paymentTerm: PaymentTerm) => {
    setEditPaymentTerm(paymentTerm);
    setFormModalMode("edit");
    setFormModalOpen(true);
  };

  const handleDelete = (paymentTerm: PaymentTerm) => {
    setDeletePaymentTerm(paymentTerm);
    setDeleteModalOpen(true);
  };

  const handleRestore = async (paymentTerm: PaymentTerm) => {
    try {
      ...
      if (!res.ok) {
        toast.error(json.error || "Failed to restore payment term");
        return;
      }
  ...
  const exportPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(16);
    doc.text("Payment Terms Report", 14, 20);

    autoTable(doc, {
      startY: 30,
      head: [["#", "Name", "Term Days", "Status", "Created At", "Updated At"]],
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Payment Terms");
    XLSX.writeFile(workbook, "payment-terms.xlsx");
  };
          ...
          data.map((item, index) => (
            <div key={item.id} className="border border-[#ddd] bg-white">
              ...
              <div className="px-4 py-3">
                ...
                <p className="text-sm text-[#333]">Term days: {item.termDays}</p>
