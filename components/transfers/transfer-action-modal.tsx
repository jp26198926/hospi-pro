...
export function TransferActionModal({
  open,
  onOpenChange,
  action,
  transfer,
  ...
  const handleConfirm = async () => {
    ...
    setLoading(true);
    try {
      let url = `/api/transfers/${transfer.id}`;
      let method = "POST";
      let body: string | undefined;

      if (action === "complete") {
        url = `/api/transfers/${transfer.id}/complete`;
        method = "POST";
                ...
                {action === "complete" && (
                  <>
                    ...
                    <strong className="text-[#337ab7]">
                      {transfer?.transNo || "this transfer"}
                    </strong>{" "}
                    as Completed? Stock will move from the from-location to the
                    to-location.
                  </>
                )}
                {action === "cancel" && (
