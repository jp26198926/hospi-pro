...
interface TransferScanBarProps {
  transferId: number;
...
export function TransferScanBar({
  transferId,
  fromLocationId,
  disabled,
  onSuccess,
}: TransferScanBarProps) {
  ...
  const autoAddFromScan = async (raw: string) => {
    ...
    try {
      ...
      const addRes = await fetch("/api/transfer-items", {
        method: "POST",
  ...
  return (
    <div className="space-y-2 rounded-sm border border-[#ddd] bg-[#fafafa] p-3">
      ...
      <p className="text-xs text-muted-foreground">
        Scan to <strong>add item automatically</strong>. Press <strong>F2</strong> to
        focus. Use <strong>5*P000001</strong> for qty (default 1). Stock is checked at
        the from-location.
      </p>
      {loading && <p className="text-xs text-muted-foreground">Processing scan...</p>}
    </div>
  );
}
