...
- **Auth**: JWT access token (15 min, `localStorage`) + refresh token (7 days, httpOnly cookie)
...
3. `npm run db:push` then `npm run db:seed-timezones`.
...
- **App settings** (`lib/settings.ts`): cached 30s in-memory helper reading `settings_app` id=1 (logo, favicon, name, storage type).
...
updatedAt timestamp         nullable    set on PUT only
deletedAt timestamp         nullable    set on DELETE, cleared on PATCH restore
```

Existing tables in `lib/db/schema.ts`: `departments`, `roles`, `users`, `pages`, `permissions`, `role_permissions`, `timezones`, `settings_app`, `settings_mail`, `settings_sms`, `settings_cloudinary`, `refresh_tokens`. Add new tables here.
...
- `db:seed-timezones` is required after a fresh push — settings pages depend on the `timezones` table.
